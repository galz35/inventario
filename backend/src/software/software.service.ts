    ...
    try {
      ...
      const queryProjects = `
                SELECT 
                    ...
                    (SELECT COUNT(*) FROM Inv_ope_proyecto_tareas t2 WHERE t2.idProyecto = p.idProyecto AND t2.estado <> 'Eliminada') as totalTasksGlobal,
                    (SELECT ISNULL(AVG(CAST(t2.porcentaje AS FLOAT)), 0) FROM Inv_ope_proyecto_tareas t2 WHERE t2.idProyecto = p.idProyecto AND t2.estado <> 'Eliminada') as progressGlobal,
                    -- Stats del Equipo (Solo tareas asignadas a miembros de mi equipo)
                    (SELECT COUNT(DISTINCT ta2.idTarea) 
                     FROM Inv_ope_proyecto_tarea_asignados ta2 
                     JOIN Inv_ope_proyecto_tareas t3 ON ta2.idTarea = t3.idTarea
