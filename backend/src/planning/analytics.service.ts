...
export class AnalyticsService {
  ...
  async getDashboardStats(managerId: number, month: number, year: number) {
    console.log(
      `[Analytics] Solicitando dashboard para ManagerID: ${managerId} (Mes: ${month}, Año: ${year})`,
    );
    try {
      ...
      const queryRaw = `
                ...
                SELECT 
                    ...
                    p.nombre,
                    p.estado,
                    (SELECT ISNULL(AVG(CAST(st.porcentaje AS FLOAT)), 0) FROM Inv_ope_proyecto_tareas st WHERE st.idProyecto = p.idProyecto AND st.estado NOT IN ('Eliminada', 'Archivada')) as globalProgress,
                ...
                LEFT JOIN Inv_ope_proyecto_tarea_asignados ta ON t.idTarea = ta.idTarea AND ta.idUsuario IN (${idsStr})
      ...
      const usersWithActiveTasks = await ejecutarQuery<{ idUsuario: number }>(`
                ...
                FROM Inv_ope_proyecto_tarea_asignados ta
                INNER JOIN Inv_ope_proyecto_tareas t ON ta.idTarea = t.idTarea
                WHERE ta.idUsuario IN (${idsStr})
                  AND t.estado IN ('Pendiente', 'EnCurso')
            `);
      ...
      const blockersDetail = await ejecutarQuery<{
        ...
        dias: number;
                ...
                LEFT JOIN Inv_seg_usuarios u ON b.idUsuario = u.idUsuario
  ...
  async getBottlenecks() {
    ...
    const delayedTasksSql = `
            ...
            FROM Inv_ope_proyecto_tareas t
            JOIN Inv_ope_proyecto_tarea_asignados ta ON t.idTarea = ta.idTarea
            JOIN Inv_seg_usuarios u ON ta.idUsuario = u.idUsuario
    ...
    const blockersSql = `
            ...
            FROM Inv_ope_proyecto_bloqueos b
            JOIN Inv_seg_usuarios u ON b.idUsuario = u.idUsuario
