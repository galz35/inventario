...
export async function obtenerProyectosPorUsuario(carnet: string) {
  ...
  return await ejecutarQuery<ProyectoDb>(
    'EXEC sp_ObtenerProyectos @carnet = @carnet',
...
export async function obtenerTodosProyectos(filter?: any) {
  ...
  return await ejecutarSP<ProyectoDb>('sInv_ope_proyectos_Listar', {
...
export async function crearProyecto(dto: {
  ...
  const res = await ejecutarQuery<{ idProyecto: number }>(
    `
        INSERT INTO Inv_ope_proyectos (nombre, descripcion, idNodoDuenio, area, subgerencia, gerencia, fechaInicio, fechaFin, fechaCreacion, idCreador, estado, tipo)
...
export async function actualizarDatosProyecto(
  ...
  await ejecutarQuery(
    `UPDATE Inv_ope_proyectos SET ${sets.join(', ')} WHERE idProyecto = @idProyecto`,
    params,
  );
}
...
export async function obtenerTareaPorId(idTarea: number) {
  ...
  >(
    `
        ...
        LEFT JOIN Inv_seg_usuarios uc ON t.idCreador = uc.idUsuario
  ...
  if (tarea) {
    ...
    try {
      const avances = await ejecutarQuery<any>(
        `
                IF OBJECT_ID(N'dbo.Inv_ope_proyecto_tarea_avances', N'U') IS NOT NULL
                SELECT idLog, idTarea, idUsuario, progreso, comentario, fecha
                FROM Inv_ope_proyecto_tarea_avances
                WHERE idTarea = @id
                ORDER BY fecha DESC
            `,
        { id: { valor: idTarea, tipo: Int } },
...
export async function actualizarTarea(
  ...
  if ((updates as any).idTareaPadre !== undefined) {
    await ejecutarQuery(
      `SET QUOTED_IDENTIFIER ON; UPDATE Inv_ope_proyecto_tareas SET idTareaPadre = @p WHERE idTarea = @t`,
      {
  ...
  if (updates.linkEvidencia) {
    try {
      await ejecutarQuery(`
                IF COL_LENGTH('Inv_ope_proyecto_tareas', 'linkEvidencia') IS NULL 
                BEGIN 
                    ALTER TABLE Inv_ope_proyecto_tareas ADD linkEvidencia NVARCHAR(MAX); 
                END
                ELSE
                BEGIN
                    -- Si existe, asegurar que sea MAX (opcional, pero útil si se creó pequeña)
                     IF (SELECT max_length FROM sys.columns WHERE object_id = OBJECT_ID('Inv_ope_proyecto_tareas') AND name = 'linkEvidencia') < 2000
                     BEGIN
                        ALTER TABLE Inv_ope_proyecto_tareas ALTER COLUMN linkEvidencia NVARCHAR(MAX);
                     END
                END
             `);
    } catch (e) {
  ...
  if (
    ...
    if (sets.length > 0) {
      await ejecutarQuery(
        `SET QUOTED_IDENTIFIER ON; UPDATE Inv_ope_proyecto_tareas SET ${sets.join(', ')} WHERE idTarea = @id`,
        params,
      );
    }
  }
...
export async function obtenerPlanPorId(idPlan: number) {
  const res = await ejecutarQuery<any>(
    'SELECT * FROM Inv_ope_planes_trabajo WHERE idPlan = @id',
...
export async function upsertPlan(datos: any) {
  ...
  const existente = await ejecutarQuery(
    `
        SELECT idPlan FROM Inv_ope_planes_trabajo 
  ...
  if (existente.length > 0) {
    ...
    const res = await ejecutarQuery<{ idPlan: number }>(
      `
            INSERT INTO Inv_ope_planes_trabajo (idUsuario, mes, anio, objetivos, estado, idCreador, fechaCreacion)
...
export async function obtenerNodosLiderados(idUsuario: number) {
  return await ejecutarQuery<{ idNodo: number }>(
    `
        SELECT idNodo FROM Inv_seg_usuariosOrganizacion 
...
export async function obtenerHijosDeNodos(idsPadres: number[]) {
  ...
  return await ejecutarQuery<{ idNodo: number }>(`
        SELECT idNodo FROM Inv_cat_organigramas WHERE idPadre IN (${idsStr})
    `);
}
...
export async function obtenerUsuariosEnNodos(idsNodos: number[]) {
  ...
  return await ejecutarQuery<{ idUsuario: number }>(`
        SELECT idUsuario FROM Inv_seg_usuariosOrganizacion WHERE idNodo IN (${idsStr})
    `);
}
...
export async function crearSolicitudCambio(dto: {
  ...
  await ejecutarQuery(
    `
        INSERT INTO Inv_ope_proyecto_solicitudes_cambio (idTarea, idUsuarioSolicitante, campo, valorAnterior, valorNuevo, motivo)
...
export async function obtenerSolicitudesPendientes() {
  return await ejecutarQuery(`
        ...
        FROM Inv_ope_proyecto_solicitudes_cambio s
        JOIN Inv_ope_proyecto_tareas t ON s.idTarea = t.idTarea
        LEFT JOIN Inv_ope_proyectos p ON t.idProyecto = p.idProyecto
        JOIN Inv_seg_usuarios u ON s.idUsuarioSolicitante = u.idUsuario
...
export async function obtenerSolicitudPorId(id: number) {
  const res = await ejecutarQuery<any>(
    'SELECT * FROM Inv_ope_proyecto_solicitudes_cambio WHERE idSolicitud = @id',
...
export async function resolverSolicitud(
  ...
  await ejecutarQuery(
    `
        UPDATE Inv_ope_proyecto_solicitudes_cambio 
        SET estado = @estado, 
...
export async function reasignarTareas(taskIds: number[], toCarnet: string) {
  await ejecutarSP('sInv_ope_proyecto_tareas_Reasignar_PorCarnet', {
...
export async function obtenerTareasCriticas(carnets: string[]) {
  ...
  // FIX: Ahora hacemos JOIN con Inv_ope_proyecto_tarea_asignados porque esa es la tabla real de asignaciones.
  ...
  return await ejecutarQuery(`
        ...
        WITH TareasEquipo AS (
            ...
            FROM dbo.Inv_ope_proyecto_tarea_asignados ta
            ...
            FROM dbo.Inv_ope_checkin_tareas ct
            INNER JOIN dbo.Inv_ope_checkins c ON c.idCheckin = ct.idCheckin
        ...
        INNER JOIN dbo.Inv_ope_proyecto_tareas t ON t.idTarea = te.idTarea
        INNER JOIN dbo.Inv_seg_usuarios u ON te.carnet = u.carnet
        LEFT JOIN dbo.Inv_ope_proyectos p ON t.idProyecto = p.idProyecto
        WHERE t.activo = 1
          AND (
