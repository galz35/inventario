/* =========================================================
   InventarioCore / MOMENTUS - STORE PROCEDURES PARA ACCESO Y VISIBILIDAD
...
BEGIN
    ...
    /* 
      ...
      - A sí mismo
      - Su subárbol jerárquico (por jefeCarnet en Inv_seg_usuarios)
      - Permisos explícitos por empleado (Inv_seg_permisos_empleado con ALLOW)
      - Permisos por área (Inv_seg_permisos_area) => todo el subárbol de la organización
      - Delegaciones activas (Veo lo que ven quienes me delegaron)
      
      EXCLUSIONES (DENY):
      - Si existe un registro DENY en Inv_seg_permisos_empleado, se remueve de la lista final.
    */
    ...
    FROM dbo.Inv_seg_delegaciones d
    ...
    FROM dbo.Inv_seg_permisos_empleado pe
    ...
    ;WITH PermAreas AS (
        ...
        FROM dbo.Inv_seg_permisos_area pa
    ...
    NodosSub AS (
        ...
        FROM dbo.Inv_cat_organigramas o
        ...
        SELECT h.id
        FROM dbo.Inv_cat_organigramas h
...
-- =========================================================
CREATE OR ALTER PROCEDURE dbo.sInv_seg_usuarios_ObtenerDetallesPorCarnets
...
BEGIN
    ...
    LEFT JOIN dbo.Inv_seg_roles r ON u.idRol = r.idRol
    ORDER BY u.nombreCompleto;
END
GO

CREATE OR ALTER PROCEDURE dbo.sInv_seg_usuarios_ObtenerCarnetPorId
    @idUsuario INT
AS
BEGIN
    SET NOCOUNT ON;
    SELECT TOP 1 carnet FROM dbo.Inv_seg_usuarios WHERE idUsuario = @idUsuario;
END
GO

CREATE OR ALTER PROCEDURE dbo.sInv_seg_usuarios_BuscarPorCarnet
    @carnet NVARCHAR(50)
AS
BEGIN
    SET NOCOUNT ON;
    SELECT TOP 1 * FROM dbo.Inv_seg_usuarios WHERE LTRIM(RTRIM(carnet)) = LTRIM(RTRIM(@carnet));
END
GO

CREATE OR ALTER PROCEDURE dbo.sInv_seg_usuarios_BuscarPorCorreo
    @correo NVARCHAR(200)
AS
BEGIN
    SET NOCOUNT ON;
    SELECT TOP 1 * FROM dbo.Inv_seg_usuarios WHERE LOWER(LTRIM(RTRIM(correo))) = LOWER(LTRIM(RTRIM(@correo)));
END
GO

CREATE OR ALTER PROCEDURE dbo.sInv_seg_usuarios_ListarActivos
AS
BEGIN
    SET NOCOUNT ON;
    SELECT * FROM dbo.Inv_seg_usuarios WHERE activo = 1 ORDER BY nombre ASC;
END
GO

CREATE OR ALTER PROCEDURE dbo.sInv_seg_usuarios_Buscar
...
BEGIN
    SET NOCOUNT ON;
    INSERT INTO dbo.Inv_seg_delegaciones (carnet_delegante, carnet_delegado, motivo, activo, creado_en)
...
BEGIN
    SET NOCOUNT ON;
    UPDATE dbo.Inv_seg_delegaciones SET activo = 0 WHERE id = @id;
END
GO
...
BEGIN
    SET NOCOUNT ON;
    SELECT * FROM dbo.Inv_seg_delegaciones WHERE activo = 1 ORDER BY creado_en DESC;
END
GO
...
BEGIN
    SET NOCOUNT ON;
    SELECT * FROM dbo.Inv_seg_delegaciones 
...
BEGIN
    SET NOCOUNT ON;
    SELECT *
    FROM dbo.Inv_seg_permisos_area
...
BEGIN
    SET NOCOUNT ON;
    INSERT INTO dbo.Inv_seg_permisos_area (
...
BEGIN
    SET NOCOUNT ON;
    UPDATE dbo.Inv_seg_permisos_area SET activo = 0 WHERE id = @id;
END
GO
...
BEGIN
    SET NOCOUNT ON;
    SELECT * FROM dbo.Inv_seg_permisos_area WHERE activo = 1 ORDER BY creado_en DESC;
END
GO
...
BEGIN
    SET NOCOUNT ON;
    SELECT *
    FROM dbo.Inv_seg_permisos_empleado
    WHERE LTRIM(RTRIM(carnet_recibe)) = LTRIM(RTRIM(@carnetRecibe))
      AND activo = 1
    ORDER BY creado_en DESC;
END
...
BEGIN
    SET NOCOUNT ON;
    INSERT INTO dbo.Inv_seg_permisos_empleado (
...
BEGIN
    SET NOCOUNT ON;
    UPDATE dbo.Inv_seg_permisos_empleado SET activo = 0 WHERE id = @id;
END
GO
...
BEGIN
    SET NOCOUNT ON;
    SELECT * FROM dbo.Inv_seg_permisos_empleado WHERE activo = 1 ORDER BY creado_en DESC;
END
GO
...
BEGIN
    ...
    IF (@id IS NULL) BEGIN SELECT TOP 0 * FROM dbo.Inv_seg_usuarios; RETURN; END;

    ;WITH NodosSub AS (
        SELECT id AS idorg FROM dbo.Inv_cat_organigramas WHERE id = @id
        UNION ALL
        SELECT n.id FROM dbo.Inv_cat_organigramas n JOIN NodosSub ns ON n.idPadre = ns.idorg
    )
    SELECT TOP (@limite) 
