...
DROP TABLE IF EXISTS Inv_seg_permisos_area CASCADE;
DROP TABLE IF EXISTS Inv_seg_permisos_empleado CASCADE;
DROP TABLE IF EXISTS Inv_seg_delegaciones CASCADE;
...
-- =============================================================================
-- 3) Inv_seg_permisos_area - Permiso por área/subárbol
-- "Este usuario puede ver este nodo y su subárbol"
-- =============================================================================
CREATE TABLE Inv_seg_permisos_area
(
...
CREATE INDEX ix_permiso_area_recibe_activo ON Inv_seg_permisos_area(carnet_recibe, activo);
CREATE INDEX ix_permiso_area_idorg ON Inv_seg_permisos_area(idorg_raiz);

COMMENT ON TABLE Inv_seg_permisos_area IS 'Permisos por área/subárbol organizacional';
COMMENT ON COLUMN Inv_seg_permisos_area.alcance IS 'SUBARBOL: incluye hijos | SOLO_NODO: solo ese nodo';


-- =============================================================================
-- 4) Inv_seg_permisos_empleado - Permiso puntual por empleado
-- "Este usuario puede ver a este empleado específico"
-- =============================================================================
CREATE TABLE Inv_seg_permisos_empleado
(
    id              BIGSERIAL PRIMARY KEY,
    carnet_otorga   VARCHAR(100) REFERENCES p_empleados(carnet),
    carnet_recibe   VARCHAR(100) NOT NULL REFERENCES p_empleados(carnet),
...
ON Inv_seg_permisos_empleado(carnet_recibe, carnet_objetivo) 
WHERE activo = TRUE;

CREATE INDEX ix_permiso_emp_recibe_activo ON Inv_seg_permisos_empleado(carnet_recibe, activo);

COMMENT ON TABLE Inv_seg_permisos_empleado IS 'Permisos puntuales por empleado específico';


-- =============================================================================
-- 5) Inv_seg_delegaciones - Delegación de visibilidad
-- "La secretaria ve lo que ve el gerente"
-- =============================================================================
CREATE TABLE Inv_seg_delegaciones
(
    id               BIGSERIAL PRIMARY KEY,
...
ON Inv_seg_delegaciones(carnet_delegante, carnet_delegado) 
WHERE activo = TRUE;

CREATE INDEX ix_delegacion_delegado_activo ON Inv_seg_delegaciones(carnet_delegado, activo);

COMMENT ON TABLE Inv_seg_delegaciones IS 'Delegación de visibilidad (ej: secretaria ve lo del gerente)';
COMMENT ON COLUMN Inv_seg_delegaciones.carnet_delegante IS 'El gerente que delega su visibilidad';
COMMENT ON COLUMN Inv_seg_delegaciones.carnet_delegado IS 'La secretaria que recibe la delegación';
...
AND table_name IN ('p_organizacion_nodos', 'p_empleados', 'Inv_seg_permisos_area', 'Inv_seg_permisos_empleado', 'Inv_seg_delegaciones')
ORDER BY table_name;
