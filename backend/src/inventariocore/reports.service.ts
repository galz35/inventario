  ...
  async getReporteProductividad(idLider: number, filter?: any) {
    try {
      ...
      const sql = `
                ...
                LEFT JOIN Inv_ope_proyecto_tarea_asignados ta ON u.idUsuario = ta.idUsuario
                LEFT JOIN Inv_ope_proyecto_tareas t ON ta.idTarea = t.idTarea
  ...
  async getReporteBloqueosTrend(idLider: number, filter?: any) {
    try {
      ...
      const sql = `
                ...
                INNER JOIN Inv_seg_usuarios u ON b.idUsuario = u.idUsuario
                WHERE u.carnet IN (SELECT value FROM STRING_SPLIT(@carnets, ','))
    ...
    try {
      ...
      const users = await ejecutarQuery(
        `
                ...
                FROM Inv_seg_usuarios u
                LEFT JOIN Inv_ope_proyecto_tarea_asignados ta ON u.idUsuario = ta.idUsuario
                LEFT JOIN Inv_ope_proyecto_tareas t ON ta.idTarea = t.idTarea AND t.estado IN ('Pendiente','EnCurso') AND t.activo = 1
                WHERE u.carnet IN (SELECT value FROM STRING_SPLIT(@carnets, ','))
    ...
    try {
      ...
      const sql = `
                SELECT 
                    ...
                    (SELECT COUNT(*) FROM Inv_ope_proyecto_bloqueos b WHERE b.idTarea IN (SELECT idTarea FROM Inv_ope_proyecto_tareas WHERE idProyecto = p.idProyecto) AND b.estado = 'Activo') as bloqueosActivos
                FROM Inv_ope_proyectos p
                LEFT JOIN Inv_ope_proyecto_tareas t ON p.idProyecto = t.idProyecto AND t.activo = 1
