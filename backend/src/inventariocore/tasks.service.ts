...
import { ProyectoFilterDto, ProyectoCrearDto } from './dto/core.dtos';
...
export class TasksService {
  ...
  async canManageUser(
    ...
    if (requesterRole === 'Admin' || requesterRole === 'Administrador')
      return true;
    ...
    if (!hasAccess) {
      console.warn(
        `[TasksService] Access Denied: Manager ${managerId} -> Subordinate ${subordinateId}`,
      );
    }
  ...
  async miDiaGet(
    ...
    const checkinHoy = await InventarioCoreRepo.obtenerCheckinPorFecha(
      carnet,
      fecha,
    );
    ...
    const tareas = await InventarioCoreRepo.getTareasUsuario(
      carnet,
      undefined,
      undefined,
      start,
    ...
    let backlog = await InventarioCoreRepo.obtenerBacklog(carnet);
  ...
  async checkinUpsert(dto: CheckinUpsertDto, carnet: string) {
    return await InventarioCoreRepo.checkinUpsert({
  ...
  async tareasMisTareas(
    ...
    return await InventarioCoreRepo.getTareasUsuario(
      carnet,
      estado,
      idProyecto,
      start,
  ...
  async tareasHistorico(carnet: string, dias: number) {
    return await InventarioCoreRepo.obtenerTareasHistorico(carnet, dias);
  }
  ...
  async tareaActualizar(idTarea: number, updates: any, idUsuario: number) {
    ...
    // Campos opcionales que asumimos existen en Inv_ope_proyecto_tareas o fallaran si no (pero son enviados por el front)
    ...
    if (updates.estado === 'Bloqueada' && motivoBloqueo) {
      await InventarioCoreRepo.bloquearTarea({
        idTarea,
  ...
  async tareaEliminar(idTarea: number, carnet: string, motivo?: string) {
    await InventarioCoreRepo.eliminarTarea(idTarea, carnet, motivo);
    return { success: true };
  }
    ...
    if (idSolicitante) {
      ...
      if (!isAdmin) {
        ...
        if (idResponsable && idResponsable !== idSolicitante) {
          const canView = await this.visibilidadService.verificarAccesoPorId(
            idSolicitante,
            idResponsable,
          );
          if (!canView)
  ...
  async equipoMiembro(idLider: number, idMiembro: number) {
    ...
    const tasks = await InventarioCoreRepo.getTareasUsuario(carnetMiembro);
  ...
  async getWorkload(carnet: string) {
    try {
      ...
      const allTasks =
        await InventarioCoreRepo.obtenerTareasMultiplesUsuarios(allCarnets);
  ...
  async tareaRevalidar(
    ...
    switch (accion) {
      ...
      case 'HechaPorOtro':
        ...
        if (idUsuarioOtro) {
          await InventarioCoreRepo.asignarUsuarioTarea(
            idTarea,
            idUsuarioOtro,
            'Ejecutor',
          );
      ...
      case 'Reasignar':
        ...
        await InventarioCoreRepo.reasignarResponsable(idTarea, idUsuarioOtro);
        break;
    }
  ...
  async proyectoListar(idUsuario: number, filter?: ProyectoFilterDto) {
    ...
    if (
      ['Admin', 'Administrador', 'SuperAdmin'].includes(usuario.rolGlobal || '')
    ) {
  ...
  private async assertCanManageProject(idProyecto: number, idUsuario: number) {
    ...
    throw new ForbiddenException(
      'No tienes permiso para gestionar este proyecto',
    );
  }
  ...
  async proyectoClonar(
    ...
    // 3. Get tasks
    const tareas =
      await InventarioCoreRepo.obtenerTareasPorProyecto(idOriginal);
  ...
  async tareasDeProyecto(idProyecto: number, idUsuario: number) {
    ...
    return await InventarioCoreRepo.obtenerTareasPorProyecto(idProyecto);
  }
  ...
  async getEquipoBacklog(idUsuarioLider: number) {
    try {
      ...
      return await InventarioCoreRepo.ejecutarQuery(
        `
  ...
  async getBloqueosUsuario(idUsuarioTarget: number) {
    return await InventarioCoreRepo.ejecutarQuery(
      `
            ...
            FROM Inv_ope_proyecto_bloqueos b
            LEFT JOIN Inv_seg_usuarios u1 ON b.idOrigenUsuario = u1.idUsuario
            LEFT JOIN Inv_seg_usuarios u2 ON b.idDestinoUsuario = u2.idUsuario
            LEFT JOIN Inv_ope_proyecto_tareas t ON b.idTarea = t.idTarea
  ...
  async getDashboardKPIs(carnet: string) {
    try {
      return await InventarioCoreRepo.obtenerKpisDashboard(carnet);
    } catch (error) {
      console.error(
  ...
  async getEquipoHoy(carnetLider: string, fecha: string) {
    try {
      ...
      return await InventarioCoreRepo.obtenerEquipoHoy(visibleMembers, fecha);
    } catch (error) {
      console.error(
        `[TasksService] Error fetching team snapshot for ${carnetLider} at ${fecha}:`,
        error,
      );
      return {
        miembros: [],
  ...
  async getEquipoInform(carnetLider: string, fecha: string) {
    try {
      const visibleMembers =
        await this.visibilidadService.obtenerCarnetsVisibles(carnetLider);
      return await InventarioCoreRepo.obtenerEquipoInforme(
        visibleMembers,
        fecha,
      );
    } catch (error) {
      console.error(
        `[TasksService] Error fetching team inform for ${carnetLider} at ${fecha}:`,
        error,
      );
      return {
        miembros: [],
  ...
  async bloqueoCrear(dto: BloqueoCrearDto) {
    return await InventarioCoreRepo.bloquearTarea(dto);
  }

  async bloqueoResolver(idBloqueo: number, body: any, carnetResolver: string) {
    await InventarioCoreRepo.resolverBloqueo(
      idBloqueo,
  ...
  async notasListar(carnet: string) {
    const notas = await InventarioCoreRepo.obtenerNotasUsuario(carnet);
  ...
  async notaCrear(carnet: string, title: string, content: string) {
    await InventarioCoreRepo.crearNota({
      carnet,
      titulo: title,
      content,
    });
  ...
  async notaActualizar(idNota: number, title: string, content: string) {
    await InventarioCoreRepo.actualizarNota(idNota, { titulo: title, content });
    return { success: true };
  }

  async notaEliminar(idNota: number) {
    await InventarioCoreRepo.eliminarNota(idNota);
    return { success: true };
  }
}
