 ...
 * Reemplaza la lógica duplicada en InventarioCore.repo y planning.repo.
 */
  ...
  const res = await ejecutarSP<{ idTarea: number }>(
    'sp_Tarea_CrearCompleta_v2',
    {
      ...
      fechaInicioPlanificada: {
        valor: params.fechaInicioPlanificada || null,
        tipo: DateTime,
      },
...
export async function actualizarTarea(
  ...
  const hayCambioPadre =
    ...
    tareaPrevia?.idPadre !== updates.idTareaPadre;
...
export async function crearAvance(dto: {
  ...
  await ejecutarQuery(`
        IF OBJECT_ID(N'dbo.Inv_ope_proyecto_tarea_avances', N'U') IS NULL
        BEGIN
            CREATE TABLE dbo.Inv_ope_proyecto_tarea_avances(
  ...
  await ejecutarQuery(
    `
        INSERT INTO Inv_ope_proyecto_tarea_avances (idTarea, idUsuario, progreso, comentario, fecha)
...
export async function eliminarAvance(idLog: number) {
  await ejecutarQuery(
    'DELETE FROM Inv_ope_proyecto_tarea_avances WHERE idLog = @id',
...
export async function asignarUsuario(
  ...
  const existe = await ejecutarQuery(
    'SELECT 1 FROM Inv_ope_proyecto_tarea_asignados WHERE idTarea = @t AND idUsuario = @u',
    {
  ...
  if (existe.length === 0) {
    await ejecutarQuery(
      `
            INSERT INTO Inv_ope_proyecto_tarea_asignados (idTarea, idUsuario, tipo, fechaAsignacion)
...
export async function reasignarResponsable(
  ...
  // 1. Quitar anteriores
  await ejecutarQuery(
    "DELETE FROM Inv_ope_proyecto_tarea_asignados WHERE idTarea = @t AND tipo = 'Responsable'",
    {
      t: { valor: idTarea, tipo: Int },
    },
  );
...
export async function obtenerTarea(idTarea: number): Promise<TareaDb | null> {
  const res = await ejecutarQuery<TareaDb>(
    'SELECT *, porcentaje as progreso FROM Inv_ope_proyecto_tareas WHERE idTarea = @id',
