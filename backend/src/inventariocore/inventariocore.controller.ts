...
@ApiTags('InventarioCore Core (Migrated)')
@ApiBearerAuth()
@UseGuards(AuthGuard('jwt'))
@Controller()
export class InventarioCoreController {
  constructor(
  ...
  async getMiDia(@Request() req, @Query() query: FechaQueryDto) {
    ...
    return this.tasksService.miDiaGet(
      carnet,
      query.fecha,
      query.startDate,
      query.endDate,
  ...
  async crearTareaRapida(@Body() dto: TareaCrearRapidaDto, @Request() req) {
    ...
    const targetUserId = dto.idUsuario
      ? Number(dto.idUsuario)
      : req.user.userId;
    ...
    if (targetUserId && targetUserId !== req.user.userId) {
      ...
      if (!canManage)
        throw new ForbiddenException(
          'No puedes crear tareas para este usuario.',
        );
  ...
  async revalidarTarea(
    @Param('id') id: number,
    @Body() body: TareaRevalidarDto,
    @Request() req,
  ) {
  ...
  async getMemberAgenda(
    @Param('targetCarnet') targetCarnet: string,
    @Query() query: FechaQueryDto,
    @Request() req,
  ) {
    const requesterCarnet =
      req.user.carnet ||
      (await this.tasksService.resolveCarnet(req.user.userId));

    if (requesterCarnet !== targetCarnet) {
      const hasAccess = await this.tasksService.canManageUserByCarnet(
        requesterCarnet,
        targetCarnet,
      );
      if (!hasAccess) {
        throw new ForbiddenException(
          'No tienes permisos para ver la agenda de este usuario.',
        );
      }
    }
    return this.tasksService.miDiaGet(
  ...
  @Delete('tareas/:id')
  @ApiOperation({
    summary: 'Eliminar tarea (Físico si es hoy y creador, sino Soft Delete)',
  })
  ...
  async registrarAvance(
    @Param('id') id: number,
    @Body() body: { progreso: number; comentario?: string },
    @Request() req,
  ) {
    return this.tasksService.registrarAvance(
      id,
      body.progreso,
      body.comentario,
      req.user.userId,
  ...
  async solicitarCambio(
    ...
    return this.tasksService.crearSolicitudCambio(
      req.user.userId,
      body.idTarea,
      body.campo,
      body.valorNuevo,
