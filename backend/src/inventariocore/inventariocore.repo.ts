...
async function resolverCarnet(idOrCarnet: number | string): Promise<string> {
  ...
  const res = await ejecutarQuery<{ carnet: string }>(
    `SELECT carnet FROM Inv_seg_usuarios WHERE idUsuario = @id`,
...
export async function getTareasUsuario(
  ...
  return await ejecutarSP('sInv_ope_proyecto_tareas_ObtenerPorUsuario', {
...
export async function obtenerEquipoHoy(
  ...
  const miembros = await ejecutarSP<any>('sInv_seg_usuarios_ObtenerPorLista', {
...
export async function obtenerCheckinPorFecha(
  ...
  const tareas = await ejecutarQuery(
    `
        ...
        FROM Inv_ope_checkin_tareas ct
        JOIN Inv_ope_proyecto_tareas t ON ct.idTarea = t.idTarea
...
export async function obtenerTareasHistorico(carnet: string, dias: number) {
  const sqlQuery = `
        ...
        LEFT JOIN Inv_ope_checkin_tareas ct ON t.idTarea = ct.idTarea
        LEFT JOIN Inv_ope_checkins c ON ct.idCheckin = c.idCheckin
...
export async function resolverBloqueo(idBloqueo: number, resolucion: string) {
  await ejecutarQuery(
    `UPDATE Inv_ope_proyecto_bloqueos SET estado = 'Resuelto', resolucion = @res, fechaResolucion = GETDATE() WHERE idBloqueo = @id`,
    {
...
export async function obtenerTareasPorProyecto(idProyecto: number) {
  return await ejecutarSP('sInv_ope_proyecto_tareas_ObtenerPorProyecto', {
...
export async function obtenerTareasMultiplesUsuarios(carnets: string[]) {
  ...
  return await ejecutarSP<any>(
    'sInv_ope_proyecto_tareas_ObtenerMultiplesUsuarios',
    {
  ...
  const sqlQuery = `
        ...
        WHERE t.activo = 1
          ...
          AND (
              ...
              EXISTS (SELECT 1 FROM Inv_ope_proyecto_tarea_asignados ta WHERE ta.idTarea = t.idTarea AND ta.carnet = @carnet)
              
              -- CASO 2: Soy el creador Y nadie más está asignado (Tarea personal huérfana)
              OR (
                  t.creadorCarnet = @carnet 
                  AND NOT EXISTS (SELECT 1 FROM Inv_ope_proyecto_tarea_asignados ta WHERE ta.idTarea = t.idTarea)
              )
              ...
              OR EXISTS (
                  SELECT 1 
                  FROM Inv_ope_checkin_tareas ct
                  INNER JOIN Inv_ope_checkins c ON ct.idCheckin = c.idCheckin
