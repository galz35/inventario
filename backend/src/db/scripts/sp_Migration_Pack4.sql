...
-- =============================================
-- MIGRACIÓN InventarioCore: PAQUETE 4 (PLANNING & RECURRENCIA)
-- Fecha: 2026-01-25
...
BEGIN
    DECLARE @idPlan INT;
    SELECT @idPlan = idPlan FROM Inv_ope_planes_trabajo WHERE idUsuario = @idUsuario AND mes = @mes AND anio = @anio;

    IF @idPlan IS NULL SELECT NULL as idPlan;
    ELSE
    BEGIN
        SELECT * FROM Inv_ope_planes_trabajo WHERE idPlan = @idPlan;
