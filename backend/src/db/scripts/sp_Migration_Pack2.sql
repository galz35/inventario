...
-- =============================================
-- MIGRACIÓN InventarioCore: PAQUETE 2 (CORREGIDO V2 - CAMPOS CARNET NATIVOS)
-- Fecha: 2026-01-25
-- =============================================
...
BEGIN
    ...
    IF @carnetDestino IS NOT NULL
        SELECT @idDestino = idUsuario FROM Inv_seg_usuarios WHERE carnet = @carnetDestino;

    IF @idOrigen IS NULL RETURN; 

    IF EXISTS (SELECT 1 FROM Inv_ope_proyecto_bloqueos WHERE idTarea = @idTarea AND estado != 'Resuelto')
    BEGIN
        SELECT idBloqueo, 1 as yaExistia FROM Inv_ope_proyecto_bloqueos WHERE idTarea = @idTarea AND estado != 'Resuelto';
        RETURN;
    END

    -- Insert
    INSERT INTO Inv_ope_proyecto_bloqueos(idTarea, idOrigenUsuario, idDestinoUsuario, destinoTexto, motivo, accionMitigacion, creadoEn, estado)
...
BEGIN
    ...
    -- Para creador, seguimos dependiendo de JOIN o subquery si no hay columna en Inv_ope_proyecto_tareas.
    ...
    WHERE (
        t.idCreador = (SELECT idUsuario FROM Inv_seg_usuarios WHERE carnet = @carnet) 
    ...
    FROM Inv_ope_proyecto_tareas t
    JOIN Inv_ope_proyectos p ON t.idProyecto = p.idProyecto
    LEFT JOIN Inv_ope_proyecto_tarea_asignados ta ON t.idTarea = ta.idTarea
    WHERE (
        t.idCreador = (SELECT idUsuario FROM Inv_seg_usuarios WHERE carnet = @carnet)
        OR ta.carnet = @carnet
