...
-- ========================================================
-- MIGRACIÓN InventarioCore: PAQUETE 5 (TAREA CREAR & REVALIDAR CARNET-FIRST)
-- Fecha: 2026-01-25
-- ========================================================
...
BEGIN
    ...
    INSERT INTO Inv_ope_proyecto_tareas (
...
BEGIN
    ...
    SELECT @idUsuario = idUsuario FROM Inv_seg_usuarios WHERE carnet = @usuarioCarnet;

    IF @idUsuario IS NULL RETURN;

    -- Upsert Checkin
    IF EXISTS (SELECT 1 FROM Inv_ope_checkins WHERE carnet = @usuarioCarnet AND CAST(fecha AS DATE) = @fecha)
    BEGIN
        SELECT @idCheckin = idCheckin FROM Inv_ope_checkins WHERE carnet = @usuarioCarnet AND CAST(fecha AS DATE) = @fecha;
        UPDATE Inv_ope_checkins 
    ...
    ELSE
    BEGIN
        INSERT INTO Inv_ope_checkins (idUsuario, carnet, fecha, entregableTexto, nota, estadoAnimo, energia, prioridad1, prioridad2, prioridad3, linkEvidencia)
    ...
    DELETE FROM Inv_ope_checkin_tareas WHERE idCheckin = @idCheckin;
    INSERT INTO Inv_ope_checkin_tareas (idCheckin, idTarea, idUsuario, carnet, tipo, fechaCreacion)
