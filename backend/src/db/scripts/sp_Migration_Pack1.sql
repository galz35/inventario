...
-- =============================================
-- MIGRACIÓN InventarioCore: PAQUETE 1 (CORREGIDO V2 - CAMPOS CARNET NATIVOS)
-- Fecha: 2026-01-25
...
BEGIN
    ...
    WHERE (
        -- t.creadorCarnet = @carnet -- Descomentar si existe columna en Inv_ope_proyecto_tareas
        t.idCreador = (SELECT idUsuario FROM Inv_seg_usuarios WHERE carnet = @carnet) -- Fallback seguro si no columna
...
BEGIN
    ...
    SELECT @idUsuario = idUsuario FROM Inv_seg_usuarios WHERE carnet = @carnetUsuario;
    
    IF @idUsuario IS NULL RETURN;

    IF @esReasignacion = 1
    BEGIN
        DELETE FROM Inv_ope_proyecto_tarea_asignados WHERE idTarea = @idTarea AND tipo = 'Responsable';
    END

    IF NOT EXISTS (SELECT 1 FROM Inv_ope_proyecto_tarea_asignados WHERE idTarea = @idTarea AND carnet = @carnetUsuario)
    BEGIN
        -- Insertamos TANTO el ID como el CARNET para mantener consistencia
        INSERT INTO Inv_ope_proyecto_tarea_asignados (idTarea, idUsuario, carnet, tipo, fechaAsignacion)
...
BEGIN
    ...
    FROM Inv_ope_proyecto_tareas t
    JOIN Inv_seg_usuarios u ON t.idCreador = u.idUsuario
    WHERE t.idTarea = @idTarea;
    
    -- Resolver ID sol (para logs de auditoria si piden ID)
    SELECT @idSolicitante = idUsuario FROM Inv_seg_usuarios WHERE carnet = @carnetSolicitante;
    ...
    UPDATE Inv_ope_proyecto_tareas 
    SET activo = 0,
