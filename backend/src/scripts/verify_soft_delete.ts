...
import {
  ejecutarQuery,
  eliminarTarea,
} from '../inventariocore/inventariocore.repo';
...
async function main() {
  ...
  const res1 = await ejecutarQuery<{ idTarea: number }>(
    `
        INSERT INTO Inv_ope_proyecto_tareas (nombre, creadorCarnet, fechaCreacion, estado, activo)
  ...
  const check1 = await ejecutarQuery(
    `SELECT * FROM Inv_ope_proyecto_tareas WHERE idTarea = @id`,
