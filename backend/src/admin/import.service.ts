...
export class ImportService {
    ...
    for (const emp of empleados) {
      try {
        ...
        const existente = await ejecutarQuery<{ idUsuario: number }>(
          `SELECT idUsuario FROM Inv_seg_usuarios WHERE correo = @correo`,
        ...
        if (existente.length > 0) {
          ...
          await ejecutarQuery(
            `
                        UPDATE Inv_seg_usuarios SET nombre = @nombre, activo = @activo WHERE correo = @correo
                    `,
            {
          ...
          const inserted = await ejecutarQuery<{ idUsuario: number }>(
            `
                        INSERT INTO Inv_seg_usuarios (nombre, correo, activo, rolGlobal)
          ...
          await ejecutarQuery(
            `
                        INSERT INTO Inv_seg_usuariosCredenciales (idUsuario, passwordHash) VALUES (@id, @hash)
                    `,
            {
    ...
    for (const nodo of nodos) {
      try {
        ...
        const existente = await ejecutarQuery<{ id: number }>(
          `SELECT id FROM Inv_cat_organigramas WHERE nombre = @nombre`,
        ...
        if (existente.length > 0) {
          await ejecutarQuery(
            `
                        UPDATE Inv_cat_organigramas SET tipo = @tipo, activo = @activo WHERE nombre = @nombre
                    `,
            {
          ...
          await ejecutarQuery(
            `
                        INSERT INTO Inv_cat_organigramas (nombre, tipo, activo) VALUES (@nombre, @tipo, 1)
                    `,
            {
              nombre: { valor: nombre, tipo: NVarChar },
              tipo: { valor: nodo.tipo, tipo: NVarChar },
  ...
  async getStats() {
    ...
    const activos = await ejecutarQuery<{ total: number }>(
      `SELECT COUNT(*) as total FROM Inv_seg_usuarios WHERE activo = 1`,
    );
    const nodos = await ejecutarQuery<{ total: number }>(
      `SELECT COUNT(*) as total FROM Inv_cat_organigramas`,
    );
